...
var elimination = function (np, grs, adv) {
  ...
  for (var r = 1; grps.length > 1; r += 1) {
    ...
    if (bestA < adv) {
      log.trace('will only advance', bestA, 'from each match in round', r);
    }
    advUsed.push(bestA);
...
var score = function (ms, adv, id, score) {
  ...
  if (rndScored) {
    ...
    // if equal points use old seeding as new seeding between ties
    var ptsDesc = $.comparing('1', -1, '0', +1);
    var top = currRnd.map(function (m) {
      return $.zip(m.p, m.m).sort(ptsDesc).slice(0, adv);
    });
    // now flatten and sort across matches
    // this essentially re-seeds players for the next round
    top = $.pluck(0, $.flatten(top).sort(ptsDesc));
...
var results = function (size, ms, advs) {
  ...
  rounds.forEach(function (rnd, k) {
    ...
    if (isDone(rnd)) {
      ...
      var ptsDesc = $.comparing('1', -1);
      var topNonAdvs = rnd.map(function (m) {
        return $.zip(m.p, m.m).sort(ptsDesc).slice(adv); // bottom 'half'
      });
      var nonAdvs = $.pluck(0, $.flatten(topNonAdvs).sort(ptsDesc));

      nonAdvs.forEach(function (p, i) {
        var idx = p - 1;
        // pos is number of players beneath + (offset in array)
        res[idx].pos = rndPs.length + 1 - (nonAdvs.length - i);
      });
    }
    else if (isReady(rnd)) {
