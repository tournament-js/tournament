...
var ffaInvalidReason = function (np, grs, adv, limit) {
  if (!Array.isArray(grs) && grs.length > 0) {
    return "grs must be an array of length numRounds";
  }
  else if (!Array.isArray(adv)) {
    return "adv must be an array";
  }
  else if (grs.length !== adv.length + 1) {
    return "grs length must equal adv length + 1";
  }
  ...
  if (np < 2) {
    return "must at least advance 2 players to the final";
  }

  var lastGs = grs[grs.length-1];
  var lastNg = Math.ceil(np / lastGs);
  if (limit > 0) { // using limits
    if (limit < 4) {
      return "must at least advance 4 players to another tournament";
    }
    if (limit >= np) {
      return "limit must be < total number of players in last round";
    }
    ...
    if (limit % lastNg !== 0) {
      ...
      return "limit must be a multiple of last rounds number of groups" + limit + "<-lim, ng->"+lastNg;
    }
  }
  else if (lastNg !== 1) {
    return "must specify rounds all up to final when not using limits";
  }
