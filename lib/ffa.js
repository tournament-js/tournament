...
var results = function (size, ms, advs) {
  ...
  for (var i = 0; i < ms.length; i += 1) {
    ...
    if (g.m) {
      ...
      for (var j = 0; j < top.length; j += 1) {
        ...
        // inc wins
        // TODO: wins should not increase when there are TIES!
        // TODO: wins should be simply max round reached, but incrementally like in KO
  ...
  rounds.forEach(function (rnd, k) {
    ...
    if (isDone(rnd)) {
      ...
      var nonAdvs = $.pluck(0, $.flatten(topNonAdvs).sort(comparePtsSeed));
...
// TODO: upcoming for missing pId case - return all/any/first in round unscored then?
