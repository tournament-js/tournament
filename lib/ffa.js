...
function FFA(numPlayers, grs, advs, opts, matches) {
  this.adv = advs; // TODO: should be called this.advs
  this.limit = opts ? opts.limit | 0 : 0;
  this.numPlayers = numPlayers;
  Base.call(this, matches || elimination(numPlayers, grs, advs, this.limit));
}
...
FFA.prototype.upcoming = function (playerId) {
  ...
  // find currentRound
  var firstUnscored = $.firstBy($.not($.get('m')), this.matches);
  if (!firstUnscored) {
    return; // tournament over
  }
  var limboRnd = this.matches.filter(function (m) {
    return m.id.r === firstUnscored.id.r;
  });

  // if player reached limboRnd, he may be waiting for generation of nextRound
  var m = $.firstBy(function (m) {
    return m.p.indexOf(playerId) >= 0 && Array.isArray(m.m);
  }, limboRnd);

  if (!m) {
    return; // player knocked out earlier
  }

  // will he advance to nextRound ?
  var adv = this.adv[m.id.r - 1];
  var advanced = $.zip(m.p, m.m).sort(T.compareZip).some(function (ps, k) {
