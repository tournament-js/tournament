...
var invalid = function (np, grs, adv, opts) {
  if (!Number.isFinite(np) || Math.ceil(np) !== np || np < 6) {
    return "number of players must be at least 6";
  }
  if (!Array.isArray(grs) || !Array.isArray(adv) || !adv.length) {
    return "adv and grs must be non-empty arrays";
  }
...
var elimination = function (np, grs, adv, limit) {
  ...
  if (invReason !== null) {
    log.error("Invalid FFA configuration %dp sizes=%j, advs=%j, opts=%j"
  ...
  for (var i = 0; i < grs.length; i += 1) {
    var a = adv[i]
      ...
      , gsActual = G.reduceGroupSize(np, gs, numGroups);
...
var unscorable = function (ms, adv, id, score, allowPast) {
...
var score = function (ms, adv, id, score) {
  ...
  var invReason = unscorable(ms, adv, id, score, true);
  if (invReason !== null) {
    log.error("failed scoring FFA match %s with %j", idString(id), score);
    log.error("reason:", invReason);
    return false;
  }
  log.trace("scoring FFA %s with %j", idString(id), score);
...
FFA.prototype.unscorable = function (id, mapScore, past) {
  return unscorable(this.matches, this.adv[id.r - 1] || 0, id, mapScore, past);
};
