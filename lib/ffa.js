...
var elimination = function (np, grs, adv) {
  ...
  for (var r = 1; grps.length > 1; r += 1) {
    ...
    // start out with a decent guess for advancers, but fallback to 1 in worst case
    var bestA = Math.max(1, adv - (grs - minsize));
...
var score = function (ms, adv, id, score) {
  ...
  if (!Array.isArray(score) || score.length !== m.p.length)  {
    log.error("scores not array of length === m.p: " + JSON.stringify(score));
    return false;
  }
  if (!score.every(Number.isFinite)) {
    log.error("scores array must be numeric: " + JSON.stringify(score));
    return false;
  }
  ...
  var rndScored = currRnd.every($.get('m')); // every m score exists
  ...
  if (rndScored) {
    ...
    // sorted overalls, serves as a replacement seed map
    top = $.pluck(0, top.sort($.comparing('1', -1)));
