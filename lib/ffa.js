...
var elimination = function (np, grs, adv) {
  ...
  var cost = function (adv2, minsize, prevNumGroups) {
    ...
    //console.log('cost of a='+adv2, ':', 2*advCloseness + 1*grpCloseness + 2*grpFillCloseness);
...
var score = function (ms, adv, id, score) {
  ...
  if (adv !== -1 && score[adv] === score[adv - 1]) {
    console.error("score ambiguous - cant find top " + adv + " from " + JSON.stringify(score));
    return false;
  }
...
// TODO: upcoming now relies on adv array, make sure it works near the final..
...
FFA.fromJSON = function (matches) {
  if (!matches.length) {
    console.error("no matches found, cannot recreate tournament");
    return;
  }
  ...
  var numRounds = $.maximum(matches.map($.getDeep('id.r')));
