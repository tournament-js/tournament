...
function KnockOut(numPlayers, koArray, matches) {
  ...
  Base.call(this, KnockOut, matches || knockout(numPlayers, koArray));
}
KnockOut.prototype = Object.create(Base.prototype);

KnockOut.parse = function (obj) {
  return Base.parse(obj, KnockOut);
};

KnockOut.fromJSON = function (matches) {
  if (!matches.length || !matches[0].p.length) {
    console.error("cannot recreate knockout tournament from \n", matches);
  }
  var kos = [];
