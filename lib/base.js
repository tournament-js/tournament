...
function Base(SubClass, ms) {
  this.type = SubClass.name;
  this.rep = SubClass.idString || $.constant('UNKNOWN');
  this.version = 1; // helpful to keep track of if toString changes output slightly
  this.matches = ms;
}
...
Base.parse = function (SubClass, obj) {
  if ('string' === typeof obj) {
    // strings are inputs when using specific entry points
    obj = JSON.parse(obj);
  }
  obj.SubClass = SubClass;
  obj.type = SubClass.name; // help migrations for now
...
// TODO: decide on something good for this
//Base.prototype.isPlayable = function (id) {
//  var m = this.findMatch(id)
//  return (m && m.p.every($.neq(Base.NONE)));
//};
