...
var scorable = function (last, p, gs, id) {
  ...
  if (m && m.p && m.p.every($.notElem([WO, NA]))) {
    ...
    return !nextM.p.every($.notElem([WO, NA])); // not all players filled in in next
  }
  return false;
};
...
var score = function (last, p, gs, id, score) {
  ...
  // 2. move winner right
  playerInsert(right(last, p, id, longLbGf), w);
...
Duel.fromGamesArray = function (games) {
...
Duel.prototype.scorable = function (id) {
  return scorable(this.lastBracket, this.p, this.games, id);
};

Duel.prototype.score = function (id, mapScore) {
  return score(this.lastBracket, this.p, this.games, id, mapScore);
};

Duel.prototype.results = function () {
  return results(this.lastBracket, this.p, this.games);
};
