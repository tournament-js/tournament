...
var results = function (ms, down) {
  ...
  for (var i = 0; i < ms.length; i += 1) {
    ...
    // though may not use them safely until the continue
    var g = ms[i]
      , isBf = (isLong && last === WB && g.id.s === LB)
      , isWbGf = (last === WB && g.id.s === WB && g.id.r === p)
      , isLbGfs = (last === LB && g.id.s === LB && g.id.r >= 2*p - 1)
      , isLongSemi = (isLong && last === WB && g.id.r === p-1)
      , canPosition = !isBf && !isWbGf && !isLbGfs && !isLongSemi
      , maxr = (last === LB && g.id.s === WB) ? down(g.id, false)[0].r : g.id.r
      , p0 = g.p[0] - 1
      , p1 = g.p[1] - 1;

    // do some basic positioning handling to the players that only reached the match
    var pls = [p0, p1].filter($.gte(0));
    for (var j = 0; j < pls.length; j += 1) {
      var pX = pls[j];
