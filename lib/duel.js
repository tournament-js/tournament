...
var eType = function (last) {
  return (last === WB) ? 'single' : 'double';
};
...
var elimination = function (size, p, last, isLong) {
  ...
  log.trace('creating', size + 'p', eType(last), 'elimination tournament');
...
var right = function (last, p, id, longLbGf) {
  ...
  var inGrandFinal = (last === WB && r === p) || (last === LB && b === LB && r === 2*p)
    , inBronze = (last === WB && b === LB)
    , inShortLbGf = (b === LB && r === 2*p - 1 && !longLbGf);

  if (inGrandFinal || inBronze || inShortLbGf) {
    return null;
  }
...
var score = function (last, p, ms, isLong, id, score) {
  ...
  if (!m) {
    ...
    log.error("cannot override score walkover'd match: " + idstr);
  }
  else if (m.p[0] === NA || m.p[1] === NA) {
    log.error("missing players - cannot score not-ready match" + idstr);
  }
  else if (!Array.isArray(score) || score.length !== 2 || !score.every(Number.isFinite)) {
    log.error("scores not a length 2 numeric array: " + JSON.stringify(score));
  }
  else if (score[0] === score[1]) {
    log.error("duel score scores cannot draw in elimination: " + JSON.stringify(score));
  }
  else {
    passed = true;
  }
  ...
  log.trace("scoring", idstr, "in", Math.pow(2, p) , "player elimination tournament");
