...
var elimination = function (numPlayers, n, last) {
  if (numPlayers < 4) {
    return [];
  }
...
var score = function (last, p, ms, id, score) {
  // 0. sanity check
  var m = $.firstBy(T.byId.bind(null, id), ms);
  if (!m) {
...
var results = function (last, p, size, ms) {
  var res = [];
...
Duel.fromGames = function (matches) {
  ...
  return (new Duel(numPlayers, last, matches));
};
...
Duel.prototype.score = function (id, mapScore) {
  return score(this.last, this.p, this.matches, id, mapScore);
};
...
Duel.prototype.results = function () {
  return results(this.last, this.p, this.numPlayers, this.matches);
};
