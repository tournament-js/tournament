...
var duelInvalidReason = function (size, last, opts) {
  if (!Number.isFinite(size) || Math.ceil(size) !== size) {
    return "duel tournament size must be a finite integer";
  }
  if (size > 1024) {
    return "duel tournaments must have less than 1024 players";
  }
  if (size < 4) {
    return "duel tournaments must have at least 4 players";
  }
  if ([WB, LB].indexOf(last) < 0) {
    return "invalid duel elimination bracket; use t.WB or t.LB";
  }
  if (opts.limit) { // TODO: possible to do I guess..
    return "duel tournaments must be the final stage in multi-stage tournaments atm";
  }
  return null;
};
...
var elimination = function (size, p, last, isLong) {
  var invReason = duelInvalidReason(size, last, {limit: 0, short: !isLong});
  if (invReason !== null) {
    log.error("invalid Duel configuration %dp in %s elimination"
...
Duel.invalidReason = duelInvalidReason;

// representation of an id is constant across all Duel tournaments
...
// TODO: this stuff could be its own module almost
var wbNames = ["WB Final", "WB Semi Finals", "WB Round of "];
var lbNames = ["Grand Final, LB Final", "LB Winning Round of ", "LB Crossover Round of "];
var roundNames = ["Finals", "Semi Finals", "Quarter Finals", "Round of "];
//var roundNames2 = ["Finals", "Semis", "Ro4", "Ro"];
var roundName = function (last, p, r, br, namesL10n) {
  // this only really works for short single elimination..
  if (br === WB) {
    var names = namesL10n || (last === WB) ? roundNames : wbNames;
    return (r > p - 3) ? names[p - r] : names[3] + Math.pow(2, p - r);
  }
  else if (br === LB) {
    if (r >= 2*p - 1)  return lbNames[0];
    if (r === 2*p - 2) return lbNames[1];
    // TODO: fix up this at some point
    return lbNames[2] + r;
  }
};

Duel.prototype.roundName = function (r, br, namesL10n) {
  return roundName(this.last, this.p, r, br, namesL10n);
};
