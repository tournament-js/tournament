...
var seeds = function (i, pow) {
  var even = evenSeed(i, pow);
...
var elimination = function (size, n, last, isLong) {
  if (size < 4) {
    return [];
  }
  ...
  var num = Math.pow(2, n)    // true model number, >= size
    , matches = [];

  log.trace('creating', size + 'p', (last === WB) ? 'single' : 'double', 'elimination tournament');
  ...
  if (last >= LB) {
    if (n > 2) { // otherwise we only need the final matches added
      for (r = 3; r <= 2*n - 2; r += 1) {
        ...
        for (g = 1; g <= Math.pow(2, n - 1 - Math.floor((r + 1) / 2)); g += 1) {
...
var right = function (last, p, id, longLbGf) {
  ...
  if (r < 1 || g < 1) {
    log.error("bad match identifier:" + representation(b, r, g));
    return null;
  }
...
var score = function (last, p, ms, isLong, id, score) {
  ...
  if (m.p[0] === WO || m.p[1] === WO) {
    log.error("cannot override score walkover'd match: " + representation(id));
    return false;
  }
  if (m.p[0] === NA || m.p[1] === NA) {
    log.error("missing players - cannot score not-ready match" + representation(id));
    return false;
  }
  if (!Array.isArray(score) || score.length !== 2)  {
    log.error("invalid scores: must be array of length 2 - got: " + JSON.stringify(score));
    return false;
  }
  if (!score.every(Number.isFinite) || score[0] === score[1]) {
    log.error("invalid player scores: both must be numeric and different - got: " + JSON.stringify(score));
    return false;
  }

  log.trace("scoring", representation(m.id), "in", Math.pow(2, p) , "player elimination tournament");
  ...
  var playerInsert = function (where, adv) {
    ...
    if (!insertM) {
      log.error("tournament corrupt: " + representation(id) + " not found!");
      return false;
    }
    insertM.p[pos] = adv;
...
// TODO: include this?
// not needed atm, results will have moved a guy to pos === 1
