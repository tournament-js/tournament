...
var results = function (ms, posAry, oldRes, limit) {
  var last = ms[ms.length-1];
  ...
  // get a partial posAry2
  var posAry2 = posByGroup2(posAry, r1);
  // loop over posAry2 and assign pos based on these with a simple tieCompute
  // between them - not always perfect in the middle of r1, but it can't ever be
  // because any system that tries to to this perfectly runs into the problem of
  // accounting for two different scoring systems - (pts,?maps) and (xplacement)
  // unscored matches uses the former, and scored ones use the latter
  // (but only at the cluster that needs to be broken)


  // ultimately, revert to xplacement ONLY
  var xarys = []; // of xplacers between groups
  posAry2.forEach(function (grp) {
    grp.forEach(function (gxp, i) {
      if (!xarys[i]) {
        xarys[i] = [];
      }
      xarys[i] = xarys[i].concat(gxp);
    });
  });

  xarys.reduce(function (currPos, xplacers) {
    // xplacer ties are split up aesthetically only by old scores, then seed
    xplacers.sort($.comparing('pts', -1, 'maps', -1, 'seed', +1));
    // but ultimately assigned a pos based on the length of xplacers
    xplacers.forEach(function (p) {
      var resEl = getEntry(p);
      resEl.pos = currPos; // all x-placers tied between now
    });
