...
var createTbForGroups = function (oldRes, limit) {
  var invReason = invalid(oldRes, limit);
  if (invReason !== null) {
...
var posByGroup2 = function (posAry, r1) {
  console.log('posAry pre-merge', posAry);
  ...
  for (var i = 0; i < posAry.length; i += 1) {
    ...
    if (!m) {
      res[i] = posG.slice(); // copy entire posAry for this group
      continue;
    }
    else {
      ...
      for (var k = 0; k < posG.length; ) {
        ...
        if (m.p.indexOf(p[0]) >= 0) {
          ...
          for (var j = 0; j < p.length; j += 1) {
            console.log('splitting up grp', i+1, 'posAryEl', k,
              'from', p, 'to', [zip[j]]);
          ...
          console.log('copying grp', i+1, 'posAryEl', k, 'which is', p);
  ...
  console.log('posAry2 (post merge)', res);

  return res;
};

var score = function (ms, oldRes, limit, id, scs) {
  // 0. error handling - if this fails client didnt guard so we log
  var invReason = unscorable(ms, id, score, true);
  if (invReason !== null) {
    console.error("failed scoring TieBreaker match %s with %j", idString(id), score);
    console.error("reason:", invReason);
    return false;
  }
