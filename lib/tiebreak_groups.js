...
var createTbForGroups = function (results, topWanted) {
  // TODO: invalid
  var res = resultsByGroup(results);
  var posAry = posByGroup(res);

  var numGroups = res.length;
  var perGroup = Math.floor(topWanted / numGroups);
  var rem = topWanted % numGroups;
  var ms = [];
  ...
  var hasR1Breakers = false;
  for (var k = 0; k < numGroups; k += 1) {
    var grpPos = posAry[k];
    var unchosen = perGroup; // need to choose this many
    if (rem > 0) {
      // If we pick one extra from SOME groups, ensure we can pick that extra one.
      // This avoids weird scenarios where we pick 2.5 per group and gpos 3 is
      // tied in a group we end up wanting to pick from.
      unchosen += 1;
    }
...
var results = function (ms, oldRes, topWanted) {
  ...
  var res = [];
  // loop through x-placers while x*numGroups <= topWanted
