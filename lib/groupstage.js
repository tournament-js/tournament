...
var groupStage = function (numPlayers, groupSize) {
  ...
  // console.log('creating', numPlayers + 'p GroupStage with group size', groupSize);
...
var score = function (ms, id, score) {
  ...
  if (invReason !== null) {
    console.error("failed scoring GroupStage match %s with %j", idString(id), score);
    console.error("reason:", invReason);
    return false;
  }
  // console.log("scoring GroupStage %s with %j", idString(id), score);
...
var results = function (ms, np, grpSize, numGroups, opts) {
  ...
  grps.forEach(function (g) {
    algs.tieCompute(g, 0, mapsBreak, function (r, pos) {
      r.gpos = pos;
  ...
  xarys.reduce(function (currPos, xplacers) {
    ...
    algs.tieCompute(xplacers, currPos, mapsBreak, function (r, pos) {
