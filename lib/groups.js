...
var score = function (ms, id, score) {
  ...
  if (!m) {
    log.error(T.representation(id) + " match not found in tournament");
  }
  else if (!Array.isArray(score) || score.length !== 2)  {
    log.error("player scores must be of form [x, y]: " + JSON.stringify(score));
  }
  else if (!score.every(Number.isFinite)) {
    log.error("player scores array must be numeric: " + JSON.stringify(score));
  }
  else {
    // 1. score given match
...
function GroupStage(numPlayers, groupSize, matches) {
  ...
  if (matches) {
    ...
    if (groupSize > numPlayers) {
      log.error("cannot create groupstage with groupSize > numPlayers");
      return;
    }
